...
namespace Bot.Comandos
{
    public class Especiais : GenericModule
    {
        ...
        public void insult()
        {
            ...
            if (!Contexto.IsPrivate && sucesso && servidor.Permissoes == PermissoesServidores.ServidorPika || new AdmsExtensions().GetAdm(new Usuarios(Contexto.User.Id)).Item1)
            {
                ...
                {
                    Contexto.Channel.SendMessageAsync(embed: new EmbedBuilder()
                            .WithDescription(StringCatch.GetString("insultFail", "**{0}** infelizmente ainda não tem nenhum insulto 😔", Contexto.User.Username))
                            .WithColor(Color.Red)
                        .Build());
                }
            }
